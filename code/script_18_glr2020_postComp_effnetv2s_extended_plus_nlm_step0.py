...
LOG_DIR = './logs/'
DEVICE = 'cuda:0'
MODEL_NAME = 'effnetv2s_in21k_81314'
...
class LandmarkDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'test':
            ...
            return torch.tensor(image), torch.tensor(row.target)
...
def get_df():
    ...
    df = pd.read_csv(os.path.join(DATA_DIR, 'train_ultimate.csv'))
    print('full data: ', df.shape)

    out_dim = df.target.nunique()
...
class EffnetV2m_Landmark(nn.Module):
    ...
    def __init__(self, out_dim, load_pretrained=True):
        ...
        self.backbone = timm.create_model('tf_efficientnetv2_s_in21k', pretrained=True)
...
# get adaptive margin
tmp = np.sqrt(1 / np.sqrt(df['target'].value_counts().sort_index().values))
...
df_valid = df[df['fold'] == 0].reset_index(drop=True).query("index % 2==0")
...
# model
model = nn.DataParallel(EffnetV2m_Landmark(out_dim=out_dim)).cuda()
...
# scheduler
scheduler = OneCycleLR(optimizer, max_lr=LR, steps_per_epoch=len(train_loader), pct_start=SCHEDULER_PEAK, epochs=NUM_EPOCHS, final_div_factor=1)
