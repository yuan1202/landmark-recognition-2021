...
df = df.loc[df['landmarks'].isna()].copy()
...
# -----------------------
embeddings = torch.zeros((len(df) , 512), dtype=torch.float16, device='cpu')

dataset = LandmarkDataset(df, transform=transforms0)
...
np.save("./embeddings/{}_test2019nonlandmark_embeddings_3scales_low".format(LOAD_MODEL), embeddings.numpy())
        
# -----------------------
embeddings = torch.zeros((len(df) , 512), dtype=torch.float16, device='cpu')

dataset = LandmarkDataset(df, transform=transforms1)
dataloader = DataLoader(dataset, batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, shuffle=False, pin_memory=True)
...
np.save("./embeddings/{}_test2019nonlandmark_embeddings_3scales_mid".format(LOAD_MODEL), embeddings.numpy())
        
# -----------------------
embeddings = torch.zeros((len(df) , 512), dtype=torch.float16, device='cpu')

dataset = LandmarkDataset(df, transform=transforms2)
dataloader = DataLoader(dataset, batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, shuffle=False, pin_memory=True)
...
np.save("./embeddings/{}_test2019nonlandmark_embeddings_3scales_high".format(LOAD_MODEL), embeddings.numpy())
