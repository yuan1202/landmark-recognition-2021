...
def get_df():
    ...
    df_train['filepath'] = df_train['id'].apply(lambda x: os.path.join(DATA_DIR, 'gldv2_full', x[0], x[1], x[2], f'{x}.jpg'))
...
class Res2Net_Landmark(nn.Module):

    def __init__(self, out_dim, load_pretrained=True):
        super(Res2Net_Landmark, self).__init__()

        self.backbone = timm.create_model('res2net101_26w_4s', pretrained=True)
...
# model
model = nn.DataParallel(Res2Net_Landmark(out_dim=out_dim)).cuda()
